<ul id="side-main-menu" class="side-menu list-unstyled">
    ...
    @if($category_permission_active || $index_permission_active || $print_barcode_active || $stock_count_active || $adjustment_active)
        ...
        <li><a href="{{url("/payments")}}" aria-expanded="false">
        ...
        <li><a href="#product" aria-expanded="false" data-toggle="collapse"> <i
            ...
            <ul id="product" class="collapse list-unstyled ">
                ...
                @if($stock_count_active)
                    <li id="stock-count-menu"><a href="{{route('stock-count.index')}}">{{trans('file.Stock Count')}}</a>
                    </li>
                @endif
            </ul>
        </li>
    ...
    @if($sale_index_permission_active || $gift_card_permission_active || $coupon_permission_active || $delivery_permission_active)
        <li><a href="#sale" aria-expanded="false" data-toggle="collapse"> <i
            ...
            <ul id="sale" class="collapse list-unstyled ">
                ...
                @endif
                     <li id="sale-create-compilimentry"><a
                            href="{{route('sale.compily')}}">Add Complimentary</a></li>
                             <li id="sale-create-compilimentry"><a
                            href="{{route('sale.compilyList')}}">Complimentary List</a></li>
            </ul>
        </li>
    @endif
        ...
        <li class=""><a href="#hrm" aria-expanded="false" data-toggle="collapse"> <i
            ...
            <ul id="hrm" class="collapse list-unstyled ">
                ...
                @if($attendance_active)
                    <li id="attendance-menu"><a href="{{route('attendance.index')}}">{{trans('file.Attendance')}}</a>
                    </li>
                @endif
                @if($payroll_active)
...
$legder_report_active = $role_has_permissions_list->where('name', 'ledger-report')->first();
    ...
    @if($profit_loss_active || $legder_report_active || $best_seller_active || $warehouse_report_active || $warehouse_stock_report_active || $product_report_active || $daily_sale_active || $monthly_sale_active || $daily_purchase_active || $monthly_purchase_active || $purchase_report_active || $sale_report_active || $sale_report_chart_active || $payment_report_active || $product_expiry_report_active || $product_qty_alert_active || $dso_report_active || $user_report_active || $customer_report_active || $supplier_report_active || $due_report_active || $supplier_due_report_active)
        <li><a href="#report" aria-expanded="false" data-toggle="collapse"> <i
            ...
            <ul id="report" class="collapse list-unstyled ">
                ...
                <li>
                    <a href="{{url("/report/activity-report")}}">Activity Report</a>
                </li>
                ...
                @endif
                 ...
                 <li id="legder-report-menu">
                        {!! Form::open(['route' => 'report.legderByDate', 'method' => 'post', 'id' => 'legder-report-form']) !!}
                        <input type="hidden" name="start_date" value="{{date('Y-m').'-'.'01'}}"/>
                        <input type="hidden" name="end_date" value="{{date('Y-m-d')}}"/>
                ...
                @endif
                      @if($user_report_active)
                 <li id="legder-report-menu">
                       {!! Form::open(['route' => 'report.ManufByDate', 'method' => 'post', 'id' => 'Manuf-report-form']) !!}
                       <input type="hidden" name="start_date" value="{{date('Y-m').'-'.'01'}}"/>
                       <input type="hidden" name="end_date" value="{{date('Y-m-d')}}"/>
                       <input type="hidden" name="OB" value="0"/>
    ...
    @if(!config('database.connections.saleprosaas_landlord'))
        ...
        {{--                <li id="addon-list"><a href="{{url('addon-list')}}"> <span>{{trans('file.Addon List')}}</span></a></li>--}}
        {{--                @if (\Schema::hasColumn('products', 'woocommerce_product_id'))--}}
        {{--                    <li><a href="{{route('woocommerce.index')}}"> <span>WooCommerce</span></a></li>--}}
        {{--                @endif--}}
        {{--            </ul>--}}
        {{--        </li>--}}
    @endif
    <li><a href="#setting" aria-expanded="false" data-toggle="collapse"> <i
        ...
        <ul id="setting" class="collapse list-unstyled ">
            ...
            @endif
            <li id="user-menu"><a
                    href="{{route('user.profile', ['id' => Auth::id()])}}">{{trans('file.User Profile')}}</a></li>
    ...
    @if(Auth::user()->role_id != 5)
        {{--        <li><a target="_blank" href="{{url('/documentation')}}"> <i--}}
