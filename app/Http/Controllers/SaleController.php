...
class SaleController extends Controller
{
   ...
   public function compilyList(Request $request)
    {
        $role = Role::find(Auth::user()->role_id);
        if($role->hasPermissionTo('sales-index')) {
            $permissions = Role::findByName($role->name)->permissions;
            ...
            return view('backend.sale.compily_list', compact('starting_date', 'ending_date', 'warehouse_id', 'sale_status', 'payment_status', 'lims_gift_card_list', 'lims_pos_setting_data', 'lims_reward_point_setting_data', 'lims_account_list', 'lims_warehouse_list', 'all_permission','options', 'numberOfInvoice', 'custom_fields', 'field_name', 'lims_courier_list'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function compilyData(Request $request)
    {
        ...
        $q = Compliy::whereDate('created_at', '>=' ,$request->input('starting_date'))->whereDate('created_at', '<=' ,$request->input('ending_date'));
        ...
        $order = 'compliys.'.$columns[$request->input('order.0.column')];
        ...
        if(empty($request->input('search.value'))) {
            $q = Compliy::with('biller', 'customer', 'warehouse', 'user')
        ...
        else
        {
            ...
            $q = Compliy::join('customers', 'compliys.customer_id', '=', 'customers.id')
                ->join('billers', 'compliys.biller_id', '=', 'billers.id')
                ->whereDate('compliys.created_at', '=' , date('Y-m-d', strtotime(str_replace('/', '-', $search))))
                ->offset($start)
                ->limit($limit)
                ->orderBy($order,$dir);
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own') {
                $q = $q->select('compliys.*')
                        ...
                        ->where('compliys.user_id', Auth::id())
                        ->orwhere([
                            ['compliys.reference_no', 'LIKE', "%{$search}%"],
                            ['compliys.user_id', Auth::id()]
                        ])
                        ->orwhere([
                ...
                foreach ($field_names as $key => $field_name) {
                    $q = $q->orwhere([
                            ['compliys.user_id', Auth::id()],
                            ['compliys.' . $field_name, 'LIKE', "%{$search}%"]
                        ]);
                }
                $sales = $q->get();
                ...
                $q = $q->select('compliys.*')
                        ->with('biller', 'customer', 'warehouse', 'user')
                        ->orwhere('compliys.reference_no', 'LIKE', "%{$search}%")
                        ->orwhere('customers.name', 'LIKE', "%{$search}%")
                        ->orwhere('customers.phone_number', 'LIKE', "%{$search}%")
                        ->orwhere('billers.name', 'LIKE', "%{$search}%");
                foreach ($field_names as $key => $field_name) {
                    $q = $q->orwhere('compliys.' . $field_name, 'LIKE', "%{$search}%");
                }
                $sales = $q->get();
                $totalFiltered = $q->count();
            }
    ...
    function allocatePayment($totalPayment, $transactions) {
        ...
        foreach ($transactions as $transaction) {
            ...
            if ($totalPayment >= $transactionAmount) {
                ...
                $totalPayment -= $transactionAmount; // Reduce the total payment
            } else {
                // Partially allocate payment
                $allocated[] = [
                    'reference_no' => $transaction->reference_no,
    ...
    if ($allocationResult['remaining_payment'] > 0) {
        ...
        return redirect()->back()->with('message', 'Payment successfully allocated.');
    }
...
public function compilyCreate()
    {
        ...
        if($role->hasPermissionTo('sales-add')) {
            ...
            $custom_fields = CustomField::where('belongs_to', 'sale')->get();
            $lims_customer_group_all = CustomerGroup::where('is_active', true)->get();
            return view('backend.sale.compily',compact('currency_list', 'lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_pos_setting_data', 'lims_tax_list', 'lims_reward_point_setting_data','options', 'numberOfInvoice', 'custom_fields', 'lims_customer_group_all'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
 ...
 public function storeC(Request $request)
    {
        ...
        try {*/
            ...
            if(isset($data['table_id'])) {
                ...
                if($latest_sale)
                   echo $data['queue'] = $latest_sale->queue + 1;
                else
                    $data['queue'] = 1;
            }
...
if (is_array($newData['product_id'])) {
    ...
    $newData['product_id'] = implode(',', $newData['product_id']);
} else {
    // If it's not an array, cast it to a string
    $newData['product_id'] = (string) $newData['product_id'];
}
$lims_sale_data = Compliy::create($newData);
            $custom_field_data = [];
            ...
            if(count($custom_field_data))
                DB::table('compliys')->where('id', $lims_sale_data->id)->update($custom_field_data);
            $lims_customer_data = Customer::find($data['customer_id']);
            $lims_reward_point_setting_data = RewardPointSetting::latest()->first();
            //checking if customer gets some points or not
            if($lims_reward_point_setting_data && $lims_reward_point_setting_data->is_active &&  $data['total_price'] >= $lims_reward_point_setting_data->minimum_amount) {
                $point = (int)($data['total_price'] / $lims_reward_point_setting_data->per_point_amount);
        ...
        $message = 'Complimentary successfully added to draft';
...
$product_batch = DB::table('product_batches')
                ->where('batch_no', $product_info[0])
                ->first();
...
if ($product_batch) {
 ...
 $product_variant_id = null;
    ...
    public function deleteByCompliy(Request $request)
    {
        ...
        foreach ($sale_id as $id) {
            $lims_sale_data = Compliy::find($id);
            // $lims_product_sale_data = Product_Sale::where('sale_id', $id)->get();
 ...
 public function CompliyDestroy($id)
    {
        $url = url()->previous();
        $lims_sale_data = Compliy::find($id);
        // $lims_product_sale_data = Product_Sale::where('sale_id', $id)->get();
        ...
        foreach ($lims_sale_data as $product_sale) {
            $lims_product_data = Product::find($product_sale->product_id);
            //adjust product quantity
