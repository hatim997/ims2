...
class ReportController extends Controller
{
    ...
    public function activityReport()
    {
        ...
        $end_date = date('Y-m-d');
        $data = Medicine_Activity::whereDate('created_at', '>=', $start_date)
            ->whereDate('created_at', '<=', $end_date)->get();


        return view("backend.report.activity_report_new",
            ["start_date" => $start_date, 'end_date' => $end_date,'data'=>$data]);
    }

    public function activityReportData(Request $request)
    {
        $data = Medicine_Activity::whereDate('created_at', '>=', $request->start_date)
...
public function ManufReportByDate(Request $request)

    {
        ...
        //$endDate = $data['end_date'];
    $ob = (int)$data['OB'];
        $debitEntries = Purchase::whereBetween('created_at', [$startDate, $endDate])->get();
        $creditEntries = Sale::whereBetween('sales.created_at', [$startDate, $endDate])
            ->join('customers', 'sales.customer_id', '=', 'customers.id')
            ->where('sales.grand_total', '>', 0)
            ->select('sales.*', 'customers.company_name as customer_name')
        ...
        $openingBalanceEntry = [
            'date' => Carbon::parse($startDate), // Use the start date as the date for the opening balance
             'reference_no' => " ",
            'description' => 'Opening Balancesss',
            'credit_amount' => 0,
            'debit_amount' => 0,
            'balance' => 0, // Set the opening balance amount if known
        ];
...
//dd($data);
    ...
    return View('backend.report.manufact_report', compact('ledgerEntries', 'startDate', 'endDate'));
